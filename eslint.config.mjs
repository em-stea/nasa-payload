...
export default tseslint.config(
  // ── Ignores ─────────────────────────────────────────────────────────────
  {
    ...
    ignores: [
      ...
      "node_modules/**",
      "next-env.d.ts",
      // Plantillas y documentación de agentes, no son código del proyecto.
      ".agents/**",
      ".claude/**",
      ".cursor/**",
      // Generados por Payload: los reescribe `payload generate`.
  ...
  // que no hace falta sumar `eslint-plugin-react-compiler` aparte.
  {
    ...
    rules: {
      // TypeScript ya valida las props.
  ...
  // ── Overrides del proyecto ──────────────────────────────────────────────
  {
    ...
    files: ["*.{js,mjs,ts}", "src/shared/utils/tw-merge/**/*.ts"],
    rules: {
      // Corren en build/dev y su salida por consola es intencional.
      "no-console": "off",
    },
  },
  // ── Prettier ────────────────────────────────────────────────────────────
    ...
    rules: {
      ...
      "prettier/prettier": [
        "warn",
        {
          ...
          tailwindFunctions: ["cva", "cx", "cn", "clsx", "twMerge"],
        },
        {usePrettierrc: false},
      ],
    },
