    ...
    >
      ...
      <ol className="flex flex-col gap-6 p-4">
        {entries.map(({ point, previous }, index) => {
          ...
          return (
            <li key={`${point.date}-${index}`} className="flex gap-3">
              ...
              <div className="flex min-w-0 flex-col gap-1 pb-2">
                <Text variant="meta.3" className="text-basic-500 uppercase">
                  ...
                  {isCurrent && <span className={cn('ms-2', accent)}>{'// Current'}</span>}
                </Text>

                <Heading as="h3" variant="title.3" className="text-primary-foreground">
                  {toEntryTitle(point)}
                </Heading>

                <Text variant="body.3" className="text-muted-foreground">
