...
export function ArticleSidebar({article}: {article: NewsArticleDetail}) {
  ...
  return (
    <aside className="flex w-full flex-col gap-4">
      ...
      <Panel action={<CheckCircle className="size-4 text-foreground" />} title="Data integrity">
        <LengthChart article={article} />

        <dl className="flex w-full flex-col gap-1">
          <Stat highlight label="Paragraphs" value={String(paragraphs).padStart(3, "0")} />
          <Stat label="Assets" value={String(article.figures.length).padStart(3, "0")} />
          <Stat last label="Read time" value={article.readingTime ?? "—"} />
        </dl>
      </Panel>
    </aside>
  );
